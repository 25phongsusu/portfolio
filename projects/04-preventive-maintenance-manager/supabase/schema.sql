...
create table public.maintenance_jobs (id bigint generated always as identity primary key, equipment_id bigint references public.equipment(id) on delete cascade, title text not null, due_date date not null, assignee uuid references auth.users(id), status maintenance_status not null default 'scheduled', faults text, replaced_parts text, generated_report text, created_at timestamptz default now());
