...
async function status(){try{const raw=(await command("systemctl",["list-units","--type=service","--all","--no-pager","--no-legend"])).trim();return raw.split("\n").filter(Boolean).map(line=>{const parts=line.trim().split(/\s+/);const name=parts[0].replace(".service","");const description=parts.slice(4).join(" ");return{name,description:description||name,state:parts[2]}})}catch{return[]}}
