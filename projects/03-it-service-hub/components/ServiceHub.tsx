...
type TicketItem = { id: number; title: string; requester: string; category: string; priority: Priority; status: Status; assignee: string; createdAt: string; history: string[]; resolutionSummary?: string };
...
const nav = [
  ...
  { id: "smart", label: "Smart Assistant", icon: Lightning }
];
...
export default function ServiceHub() {
  ...
  function updateStatus(item: TicketItem, status: Status) {
    const summary = status === "Resolved" ? `Đã xử lý ${item.category.toLowerCase()} cho ${item.requester}. Kỹ thuật viên ${item.assignee} đã hoàn tất ticket với ${item.history.length + 1} cập nhật.` : item.resolutionSummary;
    setTickets(tickets.map(t => t.id === item.id ? { ...t, status, resolutionSummary: summary, history: [...t.history, `Chuyển trạng thái sang ${status}`] } : t));
    setSelected(prev => prev ? { ...prev, status, resolutionSummary: summary, history: [...prev.history, `Chuyển trạng thái sang ${status}`] } : null); setToast("Đã cập nhật trạng thái");
  }
  ...
  if (!loggedIn) return <main className="login-page"><section className="login-story"><div className="brand-mark">R</div><p className="eyebrow">RELAY SERVICE OPERATIONS</p><h1>Mọi yêu cầu IT.<br/>Một nơi xử lý.</h1><p>Quản lý ticket, thiết bị và phản hồi theo playbook trong cùng một không gian vận hành.</p></section><form className="login-card" onSubmit={authenticate}><div><span className="kicker">{authMode === "login" ? "WELCOME BACK" : "CREATE WORKSPACE"}</span><h2>{authMode === "login" ? "Đăng nhập hệ thống" : "Đăng ký tài khoản"}</h2></div><label>Email<input name="email" type="email" defaultValue={authMode === "login" ? "admin@relay.local" : ""} required/></label><label>Mật khẩu<input name="password" type="password" minLength={8} defaultValue={authMode === "login" ? "demo1234" : ""} required/></label><label>Vai trò<select value={role} onChange={e => setRole(e.target.value as "Employee" | "Admin")}><option>Admin</option><option>Employee</option></select></label>{authError&&<p className="auth-error">{authError}</p>}<button className="primary" type="submit">{authMode === "login" ? "Đăng nhập" : "Tạo tài khoản"}</button><button className="auth-switch" type="button" onClick={()=>{setAuthMode(authMode === "login" ? "register" : "login");setAuthError("")}}>{authMode === "login" ? "Chưa có tài khoản? Đăng ký" : "Đã có tài khoản? Đăng nhập"}</button><p className="helper">Demo local. Kết nối Supabase Auth bằng file .env khi triển khai.</p></form><footer>Đây là app Demo được thực hiện bởi Phạm Đoàn Thiện Phong</footer></main>;
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {active === "smart" && <SmartAssistant tickets={tickets}/>} 
...
function SmartAssistant({tickets}:{tickets:TicketItem[]}) { const [issue,setIssue]=useState(""); const [answer,setAnswer]=useState(""); function generate(){const category=/wifi|vpn|mạng|internet/i.test(issue)?"Network":/mật khẩu|đăng nhập|account/i.test(issue)?"Account":"Software";setAnswer(`Phân loại đề xuất: ${category}\n\n1. Xác nhận phạm vi ảnh hưởng và thời điểm bắt đầu.\n2. Thu thập thông báo lỗi, phiên bản và log liên quan.\n3. Thử lại bằng cấu hình đã xác minh an toàn.\n4. Nếu chưa xử lý được, chuyển cấp kèm bằng chứng đã thu thập.\n\nPhản hồi gợi ý: Đội IT đã ghi nhận sự cố và đang kiểm tra. Chúng tôi sẽ cập nhật ngay khi có kết quả.`)} return <section className="smart-layout"><div><p className="eyebrow">SMART ASSISTANT</p><h2>Chuẩn hóa phản hồi kỹ thuật</h2><p>Áp dụng playbook cục bộ theo loại sự cố trong bản demo.</p><div className="smart-context"><Lightning size={24}/><span>{tickets.filter(t=>t.status!=="Resolved").length} ticket đang mở để tham chiếu</span></div></div><div className="panel smart-panel"><label>Mô tả sự cố<textarea value={issue} onChange={e=>setIssue(e.target.value)} placeholder="Nhập triệu chứng, phạm vi ảnh hưởng và lỗi đã thấy..."/></label><button className="primary" onClick={generate} disabled={issue.trim().length<8}>Tạo phương án xử lý</button>{answer&&<pre>{answer}</pre>}</div></section>; }
